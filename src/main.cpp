...
int main() {
	...
	std::cout << "\nОбслужено клиентов: " << stats.acceptedClients;
	std::cout << "\nОтклонено: " << stats.declinedClients;
	std::cout << "\nОбщее время работы: " << workTime << "\n";
	int id = 1;
	float sumTime = 0;
	for (auto& time : stats.cashWorkTime) {
		std::cout << "\nКасса №" << id << " работала " << time.count();
		sumTime += time.count();
		id++;
	}
	float avgTime = sumTime / cash;
	std::cout << "\n\nСреднее время работы: " << avgTime;
	std::cout << "\nСреднее время простоя: " << workTime - avgTime;
...
void PrintResult(Result res) {
	std::cout << "\nВероятность отказа: " << res.Prej;
	std::cout << "\nОтносительная пропускная способность: " << res.Q;
	std::cout << "\nАбсолютная пропускная способность: " << res.A;
	std::cout << "\nСредняя длина очереди: " << res.Lq;
	std::cout << "\nСреднее время обслуживания: " << res.t;
}
